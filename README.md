...
![Aparador d'una immobiliària](https://upload.wikimedia.org/wikipedia/commons/2/24/49_Watford_Way%2C_Hendon%2C_London%2C_United_Kingdom.png)
...
## CAS 2 D’HIBRIDACIÓ: Arquitectura i Impressió 3D.

La hibridació entre arquitectura i impressió 3D representa una revolució en la construcció i disseny d'espais. El concepte d'hibridació aquí implica la combinació de dues disciplines, l’arquitectura tradicional i la tecnologia digital de la impressió 3D.

L’arquitectura, com a disciplina, s’ha basat durant segles en l’ús de materials tradicionals (maó, ciment, fusta, acer, etc.) i en tècniques de construcció. Aquests processos són físics, és a dir, requereixen maquinària i mà d’obra. Amb l’arribada de la impressió 3D, s'ha obert una nova via per transformar aquests processos.

La impressió 3D en arquitectura implica l’ús de tecnologies digitals per "imprimir" estructures o elements arquitectònics capa per capa. Aquesta tècnica pot utilitzar una gran varietat de materials, des de plàstics fins a materials més avançats com el formigó especial per a impressores 3D.

### HIBRIDACIÓ EN EL DISSENY I LA FABRICACIÓ:

La hibridació en aquest cas es refereix a la manera com els processos de disseny i fabricació s’estan fusionant gràcies a la tecnologia de la impressió 3D. Abans, el disseny arquitectònic i la construcció eren processos separats: el disseny es feia en paper al principi i amb l’arribada de l'ordinador, amb els programes de disseny com l’AutoCAD per posar un exemple, i després es passava a la construcció física. Amb la impressió 3D, aquests dos processos es fusionen entre ells.

### LA CONSTRUCCIÓ: de l'arquitectura tradicional a la impressió 3d.

Fins fa poc, l'arquitectura es basava en tècniques de construcció físiques i materials tradicionals, que requereixen l'ús de maquinària pesant i una gran quantitat de mà d'obra. Els processos de construcció eren complexos, laboriosos i sovint lents, ja que cada component havia de ser fabricat i muntat manualment. A més, aquests processos estaven limitats per la capacitat d’adaptació dels materials i per les formes que es podien construir.

Amb l’arribada de la impressió 3D, s’ha obert una nova via que ha permès transformar aquests processos tradicionals. En lloc de construir un edifici de manera lineal i modular, la impressió 3D permet crear formes més complexes i dissenys personalitzats, ja que els elements arquitectònics es "imprimeixen" capa per capa directament des de models digitals. Aquest mètode ofereix un gran potencial per crear estructures innovadores i eficients amb menys materials i més rapidesa.

A més, la impressió 3D en arquitectura pot utilitzar una gran varietat de materials, no només plàstics o resines, sinó també materials més avançats com el formigó especial per a impressores 3D, que es pot fer servir per construir des de petites estructures fins a grans edificis. Aquesta diversitat de materials amplia les possibilitats en la creació d'espais sostenibles i adaptats a les necessitats específiques de cada projecte.

### HIBRIDACIÓ EN EL DISSENY I LA FABRICACIÓ

Una de les principals formes d’hibridació en aquest cas és la fusió dels processos de disseny i fabricació, gràcies a la tecnologia de la impressió 3D. En l'arquitectura tradicional, el procés de disseny i el de construcció estaven separats. Inicialment, els arquitectes dissenyaven en paper, després passaven a la digitalització del projecte amb programes com AutoCAD. En acabat, el disseny passava a la fase de construcció, on es realitzava la fabricació de cada component físic per mitjà de processos convencionals. Això podia generar desajustos entre la visió de l'arquitecte i el resultat final a causa de les limitacions de la fabricació.

Amb la impressió 3D, aquest procés es fusiona de manera molt més directa. Un cop els dissenyadors i arquitectes creen un model digital de l’estructura, aquest es pot enviar a una impressora 3D que, capa per capa, construeix els elements físics. Així, el disseny i la fabricació estan molt més interconnectats, i això permet una producció més precisa, ràpida i amb una gran flexibilitat per fer ajustos durant el procés. Aquest tipus d’hibridació també permet una personalització total dels dissenys, atès que la impressora pot crear formes complexes que serien difícils de realitzar amb els mètodes tradicionals.

D'aquesta manera, l'arquitectura i la impressió 3D no només es complementen, sinó que es fusionen per transformar la manera en què s'entén la construcció. Els arquitectes ja no estan limitats per les tècniques tradicionals, sinó que poden aprofitar la potència de la impressió 3D per crear formes i dissenys que abans eren inabastables. Això obre la porta a nous tipus d’espais, amb una eficiència en el disseny i la fabricació que redefineix els límits de l'arquitectura.

![Procès aditiu per a la construcció](https://upload.wikimedia.org/wikipedia/commons/8/86/Timelapse_of_3D-printer_construction_of_the_clay_house_%22Tecla%22_v1.0_%28cropped%29.gif)
